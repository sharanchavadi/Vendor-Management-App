...
Currently there is a Vendor model that has just the attributes like the name and phone_number, here are few things that needs to be done.

1. The application should have 2 attributes for keeping track of the Person name i.e. first_name & last_name instead of name. The thing is there are already quite a few people names in the system, if you decide to remove the column and add a new one, we’ll lose the data, hence change the name column to first name and add a new column for last name

2. While saving the person details, you are now expected to add the person’s email. Validation is quite important as we’ll use this to send greetings and invoices.
